...
### Диагностика конфигурации (`--check-config`)

Для проверки корректности настроек окружения и доступности локальной базы SQLite без запуска HTTP-сервера используйте флаг `--check-config`:

```sh
.venv/bin/letopis-mcp --check-config
```

Команда выполняет офлайн-валидацию конфигурации MCP, проверяет политику секретов и делает чтение-only пробу базы данных. Она не делает сетевых запросов, не открывает сетевых портов, не изменяет базу и не выводит значения секретов.
